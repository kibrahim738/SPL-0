...
//	try {
    	for(int i=0;i<value.length();i++){
			...
			if(i==0 && value.charAt(i)=='-' && (value.charAt(i+1)=='√'||value.charAt(i+1)=='s'||value.charAt(i+1)=='c'
    				||value.charAt(i+1)=='t'||value.charAt(i+1)=='l'||value.charAt(i+1)=='(')) 
			{
				stack.push((double) -1);
				op.push('*');
			}
    		...
    		else if(value.charAt(i)=='!')
    		{
    			if(!stack.isEmpty()) stack.push(factorial(stack.pop()));
    				...
    				|| ((value.charAt(i)=='-' ) && (i==0 ||value.charAt(i-1)=='('))) {
